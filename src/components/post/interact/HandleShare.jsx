...
const HandleSend = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl p-4 z-1000 border  md:absolute md:min-w-60 md:top-2 md:right-auto md:bottom-auto md:left-0 md:rounded-2xl md:p-2"
        onClick={(e) => e.stopPropagation()}
      >
        {/* core */}
        <div className="bg-gray-100 rounded-2xl md:bg-transparent">
          <div
            className="flex items-center gap-3 w-full p-3 cursor-pointer border-b md:hover:bg-gray-100 transition md:border-0 rounded-xl"
            onClick={onClose}
          >
